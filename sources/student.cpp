...
void Student::setName(const json &name) {
  Name = name.get<std::string>();
}

const std::any &Student::getGroup() const {
  return Group;
}

void Student::setGroup(const json &group) {
  Group = std::any{group};
}
...
const std::any &Student::getDebt() const {
  return Debt;
}

void Student::setDebt(const json &debt) {
  Debt = std::any{debt};
}
